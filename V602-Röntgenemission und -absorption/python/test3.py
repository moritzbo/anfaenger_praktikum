...
value = ufloat(params[0], errors[0])

print(F"ERGBENISS: {value**2}")

prozentdings = 100 - 100* (value**2/ ryddi)
print(prozentdings)

print(F"ERGBENISS: {((value)**2 *elem)/planck}")

print(ryddi*elem /planck)

ryddfreq = const.physical_constants['Rydberg constant times c in Hz'][0]

print(ryddfreq)

prozentfreq = 100 - 100* (      (((value)**2 *elem)/planck) / ryddfreq     )
print(prozentfreq)
