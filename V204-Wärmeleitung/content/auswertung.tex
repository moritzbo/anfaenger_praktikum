...
Zunächst lassen sich die Temperaturen ferner vom Peltier-Element graphisch darstellen. Dazu gehören also $T_{1}$, $T_{4}$, $T_{5}$ und $T_{8}$ also alle Temperaturen am Ende der einzelnen Metallstäbe.
...
Die beiden Messingstäbe in Abbildung \ref{fig:plot1} sind dabei zu Beginn identisch und unterscheiden sich erst ab einer Temperatur von $T = \SI{30}{\celsius}$, wobei die Temperatur am breiten Messingstab stärker wächst als die am Dünnen. Dies ist ebenfalls zu erwarten, da die Gleichung
...
Sie lassen sich mit den Werten aus Tabelle \ref{tab:lit} berechnen. 
\begin{align}
...
$T_{7}$ und $T_{8}$ ist in der Abbildung \ref{fig:plot5} gezeigt. Hier können wieder analog zum vorherigen Fall Amplituden der Temperaturen, sowie Phasendifferenzen gesucht werden. Diese Auslesedaten sind in der Tabelle
...
\subsection{Bestimmung der Frequenz und Wellenlänge}
Für die Kreisfrequenz der Temperaturwellen ergibt sich mit der Gleichung
\begin{equation}
\omega = \frac{2\pi}{T}
\end{equation}
und der eingestellten Periodendauer $T = \SI{200}{\second}$
\begin{equation*}
f = \SI{0.00314}{\hertz}.
\end{equation*}
Die Wellenlänge folgt aus dem Zusammenhang
\begin{equation}
    \label{eqn:wtf}
\lambda = \frac{v_{\text{phase}}}{2\pi \omega} 
\end{equation}
wobei sich die Phasengeschwindigkeit $v_{\text{phase}}$ aus der Gleichung \eqref{eqn:v} bestimmen lässt. Dazu werden die errechneten Wärmeleitfähigkeit $\kappa$ und die jeweiligen Literaturwerte aus Tabelle 
\ref{tab:lit} eingesetzt. Zuerst können die Gleichungen \eqref{eqn:wtf} und \eqref{eqn:v} ineinander eingesetzt werden und es folgt
\begin{equation}
\lambda = \sqrt{\frac{\kappa}{2\rho c \omega}}.
\end{equation}
In der folgenden Tabelle \ref{tab:whatever} sind die Ergebnisse der Rechnung für die unterschiedlichen Metallstäbe angegeben.
...
\begin{table}
    ...
    \begin{tabular}{c |c |c}
        \toprule
        $\lambda_{\text{messing}}$ [$\si{\meter}$]  & $\lambda_{\text{edel}}$ [$\si{\meter}$] & $\lambda_{\text{alu}}$ [$\si{\meter}$]  \\
        \midrule
        $\SI{0.0412(34)}{}$   &  $\SI{0.0095(6)}{}$   & $\SI{0.0840(90)}{}$\\
        \bottomrule
    \end{tabular}
\end{table}
